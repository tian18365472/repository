...
import com.lastproject.dim.DIMZiDuanHuoQu
...
object ZoneDimForRddV2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().config(conf).appName("ZoneDimForRddV2").master("local[1]").getOrCreate()
    ...
    logRDD.map(log =>{
      ...
      val ysqqs:List[Double] = DIMZiDuanHuoQu.qqsRtp(log.requestmode,log.processnode)
