...
object DIMZiDuanHuoQu {

  def qqsRtp(requesMode:Int,processNode :Int) = {

    if (requesMode == 1 && processNode >= 2) {
      List[Double](1,0,0)
    }else if(requesMode == 1 && processNode >= 2) {
      List[Double](1,1,0)
    }else if(requesMode == 1 && processNode >= 3) {
      List[Double](1,1,1)
    }else{
      List[Double](0,0,0)
    }
